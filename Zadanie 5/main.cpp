...
Każdy zestaw składa się z jednej dodatniej liczby całkowitej X (1 ≤ X ≤ 1000) określającej liczbę kroków wyliczonych przez Jasia.
...
using namespace std;
int *kroki,ilosc_danych;
int main()
{
    cin>>ilosc_danych;
    if(ilosc_danych<1||ilosc_danych>500)return 0;
    kroki = new int [ilosc_danych];
    for(int i=0;i<ilosc_danych;i++)
    {
        cin>>kroki[i];
        if(kroki[i]<1||kroki[i]>1000)return 0;
        cout<<kroki[i]*kroki[i]<<endl;
    }
    return 0;
}
